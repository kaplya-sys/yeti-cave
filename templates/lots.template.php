		...
		<ul class="pagination-list">
			<li class="pagination-item pagination-item-prev">
				<?php if($current_page > 1): ?>
					<a href="lots.php?category=<?= htmlspecialchars($category['code']); ?>&page=<?= $current_page - 1 ?>">Назад</a>
				<?php else: ?>
			...
			<?php for($i = 1; $i <= $pages; $i++): ?>
				<li class="pagination-item <?php if($current_page === $i) : ?>pagination-item-active<?php endif; ?>">
					<a href="lots.php?category=<?= htmlspecialchars($category['code']); ?>&page=<?= $i ?>"><?= $i ?></a>
				</li>
			<?php endfor; ?>
			<li class="pagination-item pagination-item-next">
				<?php if($current_page < $pages): ?>
					<a href="lots.php?category=<?= htmlspecialchars($category['code']); ?>&page=<?= $current_page + 1 ?>">Вперед</a>
				<?php else: ?>
